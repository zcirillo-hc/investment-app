...
- `tests/e2e/push-delivery.spec.ts:198` is intermittently flaky on the mobile project: it failed first and passed on retry in two tester runs on 2026-09-11, and passed first time in two coder runs. No change touched push code. A follow-up task to find the race was offered to the owner. Separately, the axe scan's tooltip step (`openATerm` in `tests/e2e/axe.spec.ts`) once timed out on mobile in the dark theme, waiting four minutes for a term to be stable enough to click; that attempt reported no violations, and the retry and the other three viewports passed.
