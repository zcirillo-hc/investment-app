...
11. **In e2e, read notifications only at the instant the worker's `showNotification` settles.** In Chromium `getNotifications()` is not a passive read: it deletes every stored notification the platform is not displaying that was created before the read began. Too early, and it deletes the one the worker is in the middle of showing, because a notification counts as created before it reaches the screen; `showNotification` still resolves and the notification never comes back. That is what made `push-delivery.spec.ts`'s five-payload case fail 6 runs in 30 under load, when its helpers polled straight after `deliverPushMessage`. Too late, and headless Chromium on macOS has handed it to Notification Center, which drops some within tens of milliseconds during a run, and the read deletes those too. The wrapper in that spec starts the read inside the worker as `showNotification` resolves; reuse it. Do not switch to Chromium's own message center with `--disable-features=NativeNotifications` in `launchOptions`: Playwright passes its own `--disable-features` without merging yours, and Chromium's command line keeps only the last of repeated switches by default, so the flag risks silently dropping Playwright's list. The app itself never calls `getNotifications()`.
...
- The `push-delivery.spec.ts` five-payload flake ("an unparseable payload, a wrong version and a wrong type each show one fallback", mobile) is found and fixed in the spec's helpers, not in `public/sw.js`: the helpers read `getNotifications()` while the worker was still inside `showNotification`, which in Chromium deletes the notification being shown. See gotcha 11 and the build notes entry of 2026-09-11. No other spec reads notifications (`pwa.spec.ts` only checks that `sw.js` contains `showNotification`).
- The axe scan's tooltip step (`openATerm` in `tests/e2e/axe.spec.ts`) once timed out on mobile in the dark theme, waiting four minutes for a term to be stable enough to click; that attempt reported no violations, and the retry and the other three viewports passed.
